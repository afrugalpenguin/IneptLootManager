...
ILM.CORE = LibStub("AceAddon-3.0"):NewAddon(name, "AceEvent-3.0", "AceBucket-3.0")
...
ILM.AUTOVERSION = "@project-version@"

ILM.LOG = LibStub("LibLogger"):New()

local CORE = ILM.CORE
local LOG = ILM.LOG
local MODULES = ILM.MODULES
...
local function Initialize_SavedVariables()
    if type(ILM2_DB) ~= "table" then
        ILM2_DB = {
            global = {
                ...
                logger = {
                    severity = ILM.LOG.SEVERITY.ERROR,
                    verbosity = false
                }
            }
        }
    ...
    if type(ILM2_Logs) ~= "table" then
        ILM2_Logs = {}
    end
    ILM.AF = ((GetServerTime() >= 1774915200) and (GetServerTime() <= 1775692800)) and not ILM2_DB.DisableFun2026

    StaticPopupDialogs["ILMNoFunAllowedReload"] = {
        text = "Are you sure? Will reload UI.",
        button1 = "No fun allowed :(",
        button2 = "I still want fun!",
        OnAccept = function()
            ILM2_DB.DisableFun2026 = true
            ReloadUI()
        end,
        timeout = 0,
        whileDead = 1
...
local function Initialize_Logger()
    LOG:SetSeverity(ILM2_DB.global.logger.severity)
    LOG:SetVerbosity(ILM2_DB.global.logger.verbosity)
    LOG:SetPrefix("ILM")
    LOG:SetDatabase(ILM2_Logs)
end
...
function ILM.RegisterModule(moduleName, entryPoint)
    RegisterUniversal(ILM.MODULE, moduleName, entryPoint)
end

function ILM.RegisterExternal(moduleName, entryPoint)
    RegisterUniversal(ILM.EXTERNAL, moduleName, entryPoint)
end
...
function CORE:_InitializeFeatures()
    ...
    ILM.GlobalChatMessageHandlers:Initialize() -- Initialize global chat message handlers
    ILM.GlobalUIHandlers:Initialize() -- Initialize various global misc UI handlers
    ILM.GlobalGuildChangeHandler:Initialize() -- Initialize guild change handler
...
function CORE:_InitializeExternal()
    ...
    for _,external in pairs(ILM.EXTERNAL) do
        external:Initialize()
    end
...
function CORE:_InitializeOptions()
    ...
    for _, module in pairs(ILM.OPTIONS) do
        module:Initialize()
    end
...
function CORE:_InitializeGUI()
    ...
    for _, module in pairs(ILM.GUI) do
        module:Initialize()
    end
...
function CORE:_SequentialInitialize(stageNum)
    ...
    if success then
        if stage == finalStage.name then
            LOG:Info(ILM.L["Boot complete"])
            return
        end
        stageNum = stageNum + 1
    else
        if not retry then
            error(string.format("Error Loading ILM on stage [%s]", stage))
        end
    end
...
function CORE:OnInitialize()
    ...
    ILM.MODULES.Version:Initialize()
    -- Initialize AddOn
