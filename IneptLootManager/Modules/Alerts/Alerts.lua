...
local function PointReceivedAlertFrame_SetUp(self, data)
    ...
    if data.reason ~= ILM.CONSTANTS.POINT_CHANGE_REASON.DECAY then
        ...
        self.Amount:SetText(string.format(ILM.L["%s %% %s decay"], value, suffix))
    end
...
local function BidAcceptedAlertFrame_SetUp(self, data)
    ...
    value = (value ~= nil) and tostring(value) or tostring(data.value)
    self.Amount:SetText(string.format(ILM.L["Bid %s accepted!"], value))
end
...
local function BidDeniedAlertFrame_SetUp(self, data)
    local value = tonumber(data.value)
    value = (value ~= nil) and tostring(value) or tostring(data.value)
    self.Amount:SetText(string.format(ILM.L["Bid %s denied!"], value))
end
...
eventDispatcher.addEventListener(ILM.CONSTANTS.EVENTS.USER_RECEIVED_POINTS, function(event, data)
...
eventDispatcher.addEventListener(ILM.CONSTANTS.EVENTS.USER_BID_DENIED, function(event, data)
