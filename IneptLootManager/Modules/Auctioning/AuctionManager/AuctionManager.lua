...
local AuctionInfo = ILM.MODELS.AuctionInfo
...
local function InitializeDB(self)
    self.db = ILM.MODULES.Database:Personal('auction', {
        autoAssign = true,
        autoTrade = true,
        fillFromCorpse = true,
...
local function HandleLootMessage(addon, event, message, _, _, _, playerName, ...)
    ...
    if not ILM.MODULES.RaidManager:IsInActiveRaid() then return end
    if not self:IsAuctioneer() then return end
    if GetFillAuctionListFromLootGLOnly(self) then
        if not ILM.MODULES.RaidManager:IsGroupLoot() then return end
    end
...
local function HookAuctionFilling(self)
    ILM.MODULES.Hooks:RegisterModifiedItemLinkClickHandler(function(modifiers, itemLink)
        if ILM.GlobalConfigs:GetModifierCombination() ~= modifiers then return end
        if not ILM.MODULES.RaidManager:IsInRaid() then return end

        if not itemLink then return end
        self:AddItemByLink(itemLink)
        ILM.GUI.AuctionManager:Show()
    end)
end
...
local function PostLootToRaidChat()
    ...
    if not ILM.GlobalConfigs:GetAnnounceLootToRaid() then return end
    if ILM.GlobalConfigs:GetAnnounceLootToRaidOwnerOnly() then
        if not ILM.MODULES.RaidManager:IsRaidOwner(UTILS.whoami()) then return end
    end
    ...
    for lootIndex = 1, numLootItems do
        ...
        if itemLink then
            if (tonumber(rarity) or 0) >= ILM.GlobalConfigs:GetAnnounceLootToRaidLevel() then
...
local function FillLootFromCorpse()
    ...
    if GetFillAuctionListFromCorpseMLOnly(AuctionManager) then
        if not ILM.MODULES.RaidManager:IsMasterLooter() then return end
    end
...
local function CreateConfigurationOptions(self)
    local options = {
        auctioning_header = {
            type = "header",
            name = ILM.L["Auctioning"],
            order = 30
        },
        auctioning_guild_award_announcement = {
            name = ILM.L["Announce award to Guild"],
            desc = ILM.L["Toggles loot and point award announcement to guild"],
            type = "toggle",
            set = function(i, v) ILM.GlobalConfigs:SetAnnounceAwardToGuild(v) end,
            get = function(i) return ILM.GlobalConfigs:GetAnnounceAwardToGuild() end,
            width = "double",
            order = 31
        },
        auction_default_remove_on_award = {
            name = ILM.L["Remove on award"],
            desc = ILM.L["Toggle default behavior of remove on award field in auctioning ui"],
            type = "toggle",
        ...
        auctioning_enable_auto_award_from_corpse = {
            name = ILM.L["Auto-assign from corpse"],
            desc = ILM.L["Enable loot auto-assign (Master Looter UI) from corpse when item is awarded"],
            type = "toggle",
        ...
        auctioning_enable_auto_trade = {
            name = ILM.L["Auto-trade after award"],
            desc = ILM.L["Enables auto-trade awarded loot after auctioning from bag"],
            type = "toggle",
        ...
        auctioning_list_header = {
            type = "header",
            name = ILM.L["Auctioning - List Filling"],
            order = 33.5
        },
        auction_fill_from_corpse = {
            name = ILM.L["Fill auction list from corpse"],
            desc = ILM.L["Fill auction list with items from corpse. Will happen only if you open the corpse loot window."],
            type = "toggle",
        ...
        auction_fill_from_corpse_ml_only = {
            name = ILM.L["Master Loot Only"],
            desc = ILM.L["Fill from corpse only if you are the Loot Master."],
            type = "toggle",
        ...
        auction_fill_from_loot = {
            name = ILM.L["Fill auction list with looted items"],
            desc = ILM.L["Fill auction list with looted items. This will automatically add all items you have received."],
            type = "toggle",
        ...
        auction_fill_from_loot_gl_only = {
            name = ILM.L["Group Loot Only"],
            desc = ILM.L["Fill from loot only if you are using Group Loot."],
            type = "toggle",
        ...
        auction_fill_threshold = {
            name = ILM.L["Loot rarity"],
            desc = ILM.L["Select loot rarity threshold to used to fill the auction list."],
            type = "select",
        ...
        global_auction_combination = {
            ...
            desc = ILM.L["Select modifier combination for filling auction from bags and corpse."],
            type = "select",
            values = CONSTANTS.MODIFIER_COMBINATIONS_GUI,
            sorting = CONSTANTS.MODIFIER_COMBINATIONS_SORTED,
            set = function(i, v) ILM.GlobalConfigs:SetModifierCombination(v) end,
            get = function(i) return ILM.GlobalConfigs:GetModifierCombination() end,
            order = 39
        },
        auction_remove_on_no_bids = {
            name = ILM.L["Remove items without bids"],
            desc = ILM.L["Remove items without bids from auction list. This will make marking items as disenchanted not possible."],
            type = "toggle",
        ...
        auction_disenchant_autoremoved = {
            name = ILM.L["Disenchant removed items"],
            desc = ILM.L["Automatically mark auto-removed items as disenchanted"],
            type = "toggle",
        ...
        award_post_bids = {
            name = ILM.L["Post all bids on award"],
            desc = ILM.L["Post all bids after awarding item"],
            type = "toggle",
...
local function SendAuctionStart(self)
    local message = ILM.MODELS.AuctionCommStructure:New(
        ...
        ILM.MODELS.AuctionCommStartAuction:NewFromAuctionInfo(self.currentAuction)
    )
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.AUCTION, message, CONSTANTS.COMMS.DISTRIBUTION.RAID)
end

local function SendAuctionEnd()
    local message = ILM.MODELS.AuctionCommStructure:New(CONSTANTS.AUCTION_COMM.TYPE.STOP_AUCTION, {})
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.AUCTION, message, CONSTANTS.COMMS.DISTRIBUTION.RAID)
end

local function SendAntiSnipe()
    local message = ILM.MODELS.AuctionCommStructure:New(CONSTANTS.AUCTION_COMM.TYPE.ANTISNIPE, {})
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.AUCTION, message, CONSTANTS.COMMS.DISTRIBUTION.RAID)
end
...
local function SendBidAccepted(UID, name)
    ...
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.AUCTION, message, CONSTANTS.COMMS.DISTRIBUTION.WHISPER, name, CONSTANTS.COMMS.PRIORITY.ALERT)
end

local function SendBidDenied(UID, name, reason)
    local message = ILM.MODELS.AuctionCommStructure:New(
        CONSTANTS.AUCTION_COMM.TYPE.DENY_BID,
        ILM.MODELS.AuctionCommDenyBid:New(UID, reason)
    )
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.AUCTION, message, CONSTANTS.COMMS.DISTRIBUTION.WHISPER, name, CONSTANTS.COMMS.PRIORITY.ALERT)
end
...
local function SendBidInfoInternal(auctionDistributeBidData)
    ...
    ILM.MODULES.Comms:Send(ILM.COMM_CHANNEL.BIDANNOUNCE .. tostring(currentAnnounceChannel), message, CONSTANTS.COMMS.DISTRIBUTION.RAID, nil, CONSTANTS.COMMS.PRIORITY.NORMAL)
    currentAnnounceChannel = currentAnnounceChannel + 1
    if currentAnnounceChannel == ILM.CONSTANTS.AUCTION_COMM.NUM_ANNOUNCE_CHANNELS - 1 then
...
local function SetRaidConnection(self)
    if ILM.MODULES.RaidManager:IsInRaid() then
        UpdateAuctionInfo(self, ILM.MODULES.RaidManager:GetRaid())
    end
end
...
function AuctionManager:Initialize()
    ...
    ILM.MODULES.Comms:Register(ILM.COMM_CHANNEL.BIDDING, (function(rawMessage, distribution, sender)
        local message = ILM.MODELS.BiddingCommStructure:New(rawMessage)
    ...
    ILM.MODULES.EventManager:RegisterWoWEvent({"LOOT_OPENED"}, AprilFools)
    ...
    ILM.MODULES.ConfigManager:Register(ILM.CONSTANTS.CONFIGS.GROUP.GLOBAL, CreateConfigurationOptions(self))

    self.currentAuction = AuctionInfo:New()
    self.pendingAuction = AuctionInfo:New()

    HookAuctionFilling(self)
    ILM.MODULES.EventManager:RegisterWoWEvent({"LOOT_OPENED"}, HandleLootOpenedEvent)
    ILM.MODULES.EventManager:RegisterWoWEvent({"CHAT_MSG_LOOT"}, HandleLootMessage)
...
function AuctionManager:AddItemById(itemId, callbackFn)
    if not ILM.MODULES.RaidManager:IsInRaid() then
        LOG:Message(ILM.L["Auctioning requires active raid or roster mode."])
        return
    end
...
function AuctionManager:MoveItemToPendingList(item)
    if self.currentAuction:IsInProgress() then
        LOG:Warning(ILM.L["Removing items not allowed during auction."])
        return
    end
...
local function EndAuction(self)
    ...
    for _, item in pairs(auction:GetItems()) do
        ILM.MODULES.AuctionHistoryManager:AddAuctionItem(item)
...
function AuctionManager:StopAuctionManual()
    ...
    if ILM.GlobalConfigs:GetAuctionWarning() then
        UTILS.SendChatMessage(ILM.L["Auction stopped by the Master Looter"], CHAT_MESSAGE_CHANNEL)
    end
    EndAuction(self)
end
...
local function StopAuctionTimed(self)
    ...
    self.intervalTicker:Cancel()
    if ILM.GlobalConfigs:GetAuctionWarning() then
        UTILS.SendChatMessage(ILM.L["Auction complete"], CHAT_MESSAGE_CHANNEL)
    end
    EndAuction(self)
    self:RefreshGUI()
end
...
function AuctionManager:ClearItemList()
    ...
    self.pendingAuction = AuctionInfo:New()
    ILM.GUI.AuctionManager:SetVisibleAuctionItem(nil)
    self:RefreshGUI()
end
...
function AuctionManager:StartAuction()
    ...
    if not self:IsAuctioneer() then
        LOG:Message(ILM.L["You are not allowed to auction items"])
        return
    end
    ...
    if ILM.GlobalConfigs:GetAuctionWarning() then
        ...
        if numItems > 1 then
            ...
            auctionMessage = string.format(ILM.L["Auction of %s"], auctionItem:GetItemLink())
        end
        UTILS.SendChatMessage(auctionMessage , CHAT_MESSAGE_CHANNEL)
        auctionMessage = ""
        auctionMessage = auctionMessage .. string.format(ILM.L["Auction time: %s."] .. " ", auction:GetTime())
        if auction:GetAntiSnipe() > 0 then
            auctionMessage = auctionMessage .. string.format(ILM.L["Anti-snipe time: %s."], auction:GetAntiSnipe())
        end
        UTILS.SendChatMessage(auctionMessage , CHAT_MESSAGE_CHANNEL)
        if ILM.GlobalConfigs:GetCommandsWarning() and ILM.GlobalConfigs:GetAllowChatCommands() then
    ...
    if autoItemValueOverride then
        for id, auctionItem in pairs(auction:GetItems()) do
            ILM.MODULES.RosterManager:SetRosterItemValues(auction.roster, id, auctionItem:GetValues())
        end
    end
    ...
    if not self.auctionTickerCallbacks then
        self.auctionTickerCallbacks = GenerateIntervalHandlerCallbacks({
            init = (function()
                self.bidInfoSender = ILM.MODELS.BidInfoSender:New(SENDING_INTERVAL, SendBidInfoInternal)
            end),
    ...
    NewIntervalHandlers(self,
        ILM.GlobalConfigs:GetCountdownWarning() and 5 or 0,
...
local function handleIncomingRoll(_, _, message, ...)
    ...
    local rollerProfile = ILM.MODULES.ProfileManager:GetProfileByName(UTILS.Disambiguate(who))
...
function AuctionManager:StartRoll(auctionItem)
    ...
    local message = string.format(ILM.L["Accepting rolls on %s for %s %s"], auctionItem:GetItemLink(), rollTime, ILM.L["seconds"])
    UTILS.SendChatMessage(message, CHAT_MESSAGE_CHANNEL)

    auction:Roll(auctionItem)

    self.unregisterRolls =  ILM.MODULES.EventManager:RegisterWoWEvent({"CHAT_MSG_SYSTEM"}, handleIncomingRoll)
    NewIntervalHandlers(self,
        ILM.GlobalConfigs:GetCountdownWarning() and 5 or 0,
...
function AuctionManager:StopRoll()
    ...
    UTILS.SendChatMessage(ILM.L["Rolling complete"], CHAT_MESSAGE_CHANNEL)
end
...
function AuctionManager:HandleSubmitBid(data, sender)
    ...
    local response = ILM.MODELS.UserResponse:New(data:Value(), data:Type(), data:Items())
...
local function AnnounceBid(auction, uid, name, userResponse, newHighBid)
    ...
    if not ILM.GlobalConfigs:GetBidsWarning() then return end
...
function AuctionManager:Award(item, name, price)
    ...
    local success, uuid = ILM.MODULES.LootManager:AwardItem(self.currentAuction:GetRaid(), name, item:GetItemLink(), item:GetItemID(), item:GetExtraPayload(), price, true)
    if success then
        ILM.MODULES.AuctionHistoryManager:CorrelateWithLoot(item:GetItemLink(), self.currentAuction:GetEndTime(), uuid)
        if GetPostAllBidsOnAward(self) then
            ILM.MODULES.AuctionHistoryManager:PostByUUID(uuid)
        end
        ILM.MODULES.AutoAssign:Handle(item:GetItemLink(), name)
        RevalidateBids(self)
    end
end
...
function AuctionManager:Disenchant(item)
    ...
    local success, uuid = ILM.MODULES.LootManager:DisenchantItem(self.currentAuction:GetRaid(), item:GetItemLink(), true)
    if success then
        ILM.MODULES.AuctionHistoryManager:CorrelateWithLoot(item:GetItemLink(), self.currentAuction:GetEndTime(), uuid)
        local disenchanter = ILM.MODULES.RaidManager:GetDisenchanter()
        if disenchanter then
            ILM.MODULES.AutoAssign:Handle(item:GetItemLink(), disenchanter)
        end
    end
end
...
function AuctionManager:IsAuctioneer(name, relaxed)
    ...
    return ILM.MODULES.RaidManager:IsAllowedToAuction(name, relaxed)
end
...
function AuctionManager:RefreshGUI()
    ILM.GUI.AuctionManager:Refresh(true)
end
...
CONSTANTS.AUCTION_COMM = {
    ...
    DENY_BID_REASONS_STRING = {
        [1] = ILM.L["Not in a roster"],
        [2] = ILM.L["Bidding while below minimum standings not allowed"],
        [3] = ILM.L["Bidding over current standings not allowed"],
        [4] = ILM.L["Bid too low"],
        [5] = ILM.L["Bid too high"],
        [6] = ILM.L["Invalid bid value"],
        [7] = ILM.L["Bid increment too low"],
        [8] = ILM.L["No auction in progress"],
        [9] = ILM.L["Bid cancelling not allowed"],
        [10] = ILM.L["Passing after bidding not allowed"],
        [11] = ILM.L["Off-spec bidding not allowed"],
        [12] = ILM.L["Invalid item"],
        [13] = ILM.L["Changing bid from Main-spec to Off-Spec not allowed"]
    }
}

ILM.MODULES.AuctionManager = AuctionManager
--@do-not-package@
function AuctionManager:FakeBids()
    ...
    if ILM.MODULES.RaidManager:IsInRaid() and self:IsAuctionInProgress() then
        ...
        for _=1,numBids do
            local bidder = ILM.MODULES.ProfileManager:GetProfileByGUID(profiles[math.random(1, #profiles)]):Name()
