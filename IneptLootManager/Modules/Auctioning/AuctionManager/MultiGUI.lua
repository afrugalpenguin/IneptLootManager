...
local ITEM_REGISTRY = "ilm_am_item_options"
local AUCTION_REGISTRY = "ilm_am_auction_options"
local AWARD_REGISTRY = "ilm_am_award_options"
...
local tooltip = CreateFrame("GameTooltip", "ILMAuctionManagerGUIDialogTooltip", UIParent, "GameTooltipTemplate")

local function InitializeDB(self)
    self.db = ILM.MODULES.Database:GUI('auction2', {
...
---@diagnostic disable-next-line: cast-local-type
    ...
    local options = {
        ...
        note = {
            ...
            set = (function(i,v)
                if auctionItem then
                    ILM.MODULES.AuctionManager:SetItemNote(auctionItem, tostring(v))
                end
            end),
            get = (function(i)
                local note = ""
    ...
    if ILM.MODULES.RaidManager:IsInRaid() then
        auction = ILM.MODULES.AuctionManager:GetCurrentAuctionInfo()
    end
    local order = 4
            ...
            if dataRow then
                ...
                if button == "RightButton" then
                    ...
                    if auction:IsInProgress() then
                        LOG:Message(ILM.L["Removing items not allowed during auction."])
                    else
                        if IsControlKeyDown() then
                            LOG:Message(ILM.L["Removing %s from current queue."], item:GetItemLink())
                            ILM.MODULES.AuctionManager:RemoveItemFromCurrentAuction(item)
                            self.auctionItem = nil
                        else
                            LOG:Message(ILM.L["Moving %s from current queue to pending queue."], item:GetItemLink())
                            ILM.MODULES.AuctionManager:MoveItemToPendingList(item)
                        end
                        self:Refresh()
                    end
                else
...
local function GenerateAwardOptions(self)
    local btnAwardText = ILM.L["Award"]
    ...
    local options = {
        clear = {
            name = ILM.L["Clear"],
            desc = ILM.L["Remove all items from auction list. Populates new one if there are any pending."],
            type = "execute",
            func = (function() ILM.MODULES.AuctionManager:ClearItemList() end),
            confirm = true,
            disabled = disableInProgress,
            width = 0.5,
            order = 0,
        ...
        clear_toggle = {
            ...
            -- name = ILM.L["Remove"],
            desc = ILM.L["Remove item from auction list after it's awarded."],
            type = "toggle",
        ...
        award = {
            ...
            type = "execute",
            func = (function()
                ILM.MODULES.AuctionManager:Award(self.auctionItem, self.awardPlayer, self.awardPrice)
                self.BidList:ClearSelection()
                if self.removeOnAward then
                    self.auctionItem:DecrementTotal()
                    if self.auctionItem:GetTotal() == 0 then
                        ILM.MODULES.AuctionManager:RemoveItemFromCurrentAuction(self.auctionItem)
                        self.auctionItem = nil
                    end
                end
        ...
        disenchant = {
            name = ILM.L["Disenchant"],
            type = "execute",
            func = (function()
                ILM.MODULES.AuctionManager:Disenchant(self.auctionItem)
                self.BidList:ClearSelection()
                if self.removeOnAward then
                    self.auctionItem:DecrementTotal()
                    if self.auctionItem:GetTotal() == 0 then
            ...
            confirm = (function()
                return string.format(ILM.L["Are you sure, you want to disenchant %s?"], self.auctionItem:GetItemLink())
            end),
            control = "ILMIconNoLabel",
            width = 0.2,
            order = 6,
            image = "Interface\\Buttons\\UI-GroupLoot-DE-Up",
            -- image = "Interface\\AddOns\\IneptLootManager\\Media\\Buttons\\delete2.tga",
            disabled = genericDisable,
        }
    }
    return options
    ...
    BidList:RegisterEvents({
        ...
        OnEnter = (function (rowFrame, cellFrame, data, cols, row, realrow, column, table, ...)
            ...
            if rowData and rowData.invalidReason then
                ...
                tooltip:AddLine(UTILS.ColorCodeText(CONSTANTS.AUCTION_COMM.DENY_BID_REASONS_STRING[rowData.invalidReason] or ILM.L["Unknown"], "ee4444"))
                tooltip:Show()
            end
        ...
        OnClick = function(rowFrame, cellFrame, data, cols, row, realrow, column, table, button, ...)
            ...
            if selected then
                ...
                local multiplier = ILM.MODULES.AuctionManager:GetCurrentAuctionInfo():GetAwardMultiplier(ST_GetPlayerClass(selected), self.auctionItem:GetItemID())
...
local function GenerateAuctionOptions(self)
    local options = {
        ...
        time_auction = {
            ...
            type = "input",
            set = (function(i,v) ILM.MODULES.AuctionManager:SetAuctionTime(v) end),
            get = (function(i) return tostring(ILM.MODULES.AuctionManager:GetAuctionTime()) end),
            disabled = genericDisable,
            pattern = "%d+",
            width = 0.5,
            order = 2
        ...
        time_antiSnipe = {
            name = ILM.L["Anti-snipe"],
            type = "input",
            set = (function(i,v) ILM.MODULES.AuctionManager:SetAntiSnipe(v) end),
            get = (function(i) return tostring(ILM.MODULES.AuctionManager:GetAntiSnipe()) end),
            disabled = genericDisable,
            pattern = "%d+",
            width = 0.5,
            order = 3
        ...
        auction = {
            name = (function() return ILM.MODULES.AuctionManager:IsAuctionInProgress() and ILM.L["Stop"] or ILM.L["Start"] end),
            type = "execute",
            func = (function()
                if ILM.MODULES.AuctionManager:IsAuctionInProgress() then
                    ILM.MODULES.AuctionManager:StopAuctionManual()
                else
                    ILM.MODULES.AuctionManager:StartAuction()
                end
                self:Refresh()
            end),
            width = 0.8,
        ...
        roll = {
            name = (function() return (ILM.MODULES.AuctionManager:IsAcceptingRolls() and ILM.L["Stop"] or ILM.L["Start"])  .. " " .. ILM.L["Roll"] end),
            type = "execute",
            func = (function()
                if ILM.MODULES.AuctionManager:IsAcceptingRolls() then
                    ILM.MODULES.AuctionManager:StopRoll()
                else
                    ILM.MODULES.AuctionManager:StartRoll(self.auctionItem)
                end
            end),
            control = "ILMIconNoLabel",
            width = 0.2,
            ...
            disabled = (function()
                return ILM.MODULES.AuctionManager:GetCurrentAuctionInfo():IsEmpty() or
                        not self.auctionItem or
                        ILM.MODULES.AuctionManager:IsAuctionInProgress()
            end)
        },
        bidding_header = {
            name = ILM.L["Bidding"],
        ...
        --     desc = (function()
        --         -- Legend
        --         if not ILM.MODULES.RaidManager:IsInActiveRaid() or self.raid == nil then return ILM.L["Not in raid"] end
        ...
        --                 for i= 1, count do
        --                     local profile = ILM.MODULES.ProfileManager:GetProfileByName(dataList[i])
        ...
        --         end)
        --         for p,_ in pairs(ILM.MODULES.AuctionManager:Bids()) do
        --             didAnyAction[p] = true
        --         end
        --         -- passess list
        --         local _, passed = _generateInfo(
        --             ILM.MODULES.AuctionManager:Passes(),
        --                                     { ILM.MODULES.AuctionManager:Bids() },
        --                                     "Passed")
        --         -- cant use actions
        --         local _, cantUse = _generateInfo(
        --             ILM.MODULES.AuctionManager:CantUse(),
        --                                         { ILM.MODULES.AuctionManager:Bids(), ILM.MODULES.AuctionManager:Passes() },
        --                                         "Can't use")
        --         -- closed actions
        --         local _, closed = _generateInfo(ILM.MODULES.AuctionManager:Hidden(),
        --                                     { ILM.MODULES.AuctionManager:Bids(), ILM.MODULES.AuctionManager:Passes(), ILM.MODULES.AuctionManager:CantUse() },
        --                                     "Closed")
        --         -- no action
        --         local raidersDict = {}
        ...
        --                             { ILM.MODULES.AuctionManager:Bids(), ILM.MODULES.AuctionManager:Passes(), ILM.MODULES.AuctionManager:CantUse(), ILM.MODULES.AuctionManager:Hidden() },
        --                             "No action", true)
...
local function Create(self)
    ...
    f:SetTitle(ILM.L["Auctioning"])
    f:SetLayout("table")
    f:EnableResize(false)
    f:SetHeight(570)
    f:SetWidth(BASE_WIDTH)
    self.top = f
    UTILS.MakeFrameCloseOnEsc(f.frame, "ILM_Auctioning_MultiGUI")
    ...
    self.awardPrice = 0
    self.removeOnAward = ILM.MODULES.AuctionManager:GetDefaultRemoveOnAward()
...
local function RegisterSlash(self)
    local options = {
        auction = {
            type = "execute",
            name = "Auctioning",
            desc = ILM.L["Toggle Auctioning window display"],
            handler = self,
            func = "Toggle",
        }
    }
...
function AuctionManagerGUI:BuildColumns()
    ...
    local columns = {
        ...
        {name = ILM.L["Current"],  width = 80, color = {r = 0.93, g = 0.70, b = 0.13, a = 1.0},
...
function AuctionManagerGUI:Refresh()
    ...
    if item then
        ...
        for name, response in pairs(item:GetAllResponses()) do
            ...
            local include = true
            if (response:Type() == CONSTANTS.BID_TYPE.CANCEL and not ILM.MODULES.AuctionManager:GetIncludeCancels()) or
               (response:Type() == CONSTANTS.BID_TYPE.PASS and not ILM.MODULES.AuctionManager:GetIncludePasses()) then
                include = false
            end
            if include then
...
function AuctionManagerGUI:Toggle()
    ...
    if not self._initialized then return end
    if self.top:IsVisible() or not ILM.MODULES.ACL:IsTrusted() then
        self.top:Hide()
    else
        self:Show()
    end
...
ILM.GUI.AuctionManager = AuctionManagerGUI
