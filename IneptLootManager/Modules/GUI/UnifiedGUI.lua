...
local VERTICAL_REGISTRY   = "ilm_unifiedgui_gui_options_vertical"
local HORIZONTAL_REGISTRY = "ilm_unifiedgui_gui_options_horizontal"

local _, _, _, isElvUI = UTILS.GetAddOnInfo("ElvUI")

local function InitializeDB(self)
    self.db = ILM.MODULES.Database:GUI('unifiedgui', {
...
local function CreateTabsWidget(self, content)
    ...
    for name, tab in pairs(self.tabs) do
        ...
        tabs[#tabs + 1] = {
            value = name,
            text = tabIcon .. ILM.L[UTILS.capitalize(name)],
            order = tab.order
        }
    end
...
local function CreateTabsContent(self)
    ...
    for name,_ in pairs(self.tabs) do
        self.aceObjects.scrollingTables[name] = AceGUI:Create("ILMLibScrollingTable")
    end
    ...
    AceConfigRegistry:RegisterOptionsTable("loading_banner_cfg",
    {
        ...
        args = {
            ...
            bottom = {
                ...
                fontSize = "large",
                type = "description",
                image = "Interface\\AddOns\\IneptLootManager\\Media\\Bars\\Gloss.tga",
...
function UnifiedGUI:Initialize()
    ...
    ILM.MODULES.LedgerManager:RegisterOnUpdate(function(lag, uncommitted)
        if lag ~= 0 or uncommitted ~= 0 then
            ...
            if ILM.MODULES.LedgerManager:IsTimeTraveling() then
                percentage = ILM.MODULES.LedgerManager:TimeTravelProgress()
            else
                local count = ILM.MODULES.LedgerManager:Length() + uncommitted
    ...
    ILM.MODULES.LedgerManager:RegisterOnRestart(function()
        if ILM.MODULES.LedgerManager:Length() > 0 then
            self.aceObjects.loadingBanner:Show()
        end
    end)

    ILM.MODULES.EventManager:RegisterWoWEvent({"PLAYER_LOGOUT"},
    (function()
        StoreLocation(self)
        for _, tab in pairs(self.tabs) do
            tab.handlers.store()
    ...
    ILM.MODULES.EventManager:RegisterEvent("ILM_UI_RESIZE", (function(event, data)
...
function UnifiedGUI:CreateAceGUIStructure()
    ...
    f:SetTitle(ILM.L["Inept Loot Manager"])
    f:SetLayout("Fill")
    ...
    UTILS.MakeFrameCloseOnEsc(f.frame, "ILM_UNIFIED_GUI")
    -- Widget
    CreateTabsContent(self)
    CreateTabsWidget(self, self.aceObjects.tabularContent)
    RestoreLocation(self)
    f.frame:SetScale(ILM.GlobalConfigs:GetUIScale())
    for _, tab in pairs(self.tabs) do
...
function UnifiedGUI:RegisterSlash()
    local options = {
        gui = {
            type = "execute",
            name = "Standings",
            desc = ILM.L["Toggle standings window display"],
            handler = self,
            func = "Toggle",
        }
    }
