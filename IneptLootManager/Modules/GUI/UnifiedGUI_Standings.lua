...
local UnifiedGUI_Standings = {
    ...
    nil, 2),
    tooltip = CreateFrame("GameTooltip", "ILMUnifiedGUIStandingsDialogTooltip", UIParent, "GameTooltipTemplate"),
}
...
local function GenerateAssistantOptions(self)
    return {
        ...
        award_dkp_note = {
            name = ILM.L["Note"],
            desc = (function()
                local n = ILM.L["Note to be added to award. Max 25 characters. It is recommended to not include date nor selected reason here. If you will input encounter ID it will be transformed into boss name."]
        ...
        award_dkp_value = {
            ...
            desc = ILM.L["Points value that will be awarded."],
            type = "input",
        ...
        award_dkp = {
            name = ILM.L["Award"],
            desc = ILM.L["Award points to players based on context."],
            type = "execute",
            ...
            func = (function(i)
                ...
                if self.context == CONSTANTS.ACTION_CONTEXT.RAID then
                    if ILM.MODULES.RaidManager:IsInRaid() then
                        ...
                        ILM.MODULES.PointManager:UpdateRaidPoints(raid, awardValue, awardReason, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, self.note, self.awardType)

                        if ILM.GlobalConfigs:GetAnnounceAwardToGuild() then
                            UTILS.SendChatMessage(
                                string.format(
                                    ILM.L["Awarded %s points for %s to all players in raid %s"] .. note,
                ...
                elseif self.context == CONSTANTS.ACTION_CONTEXT.ROSTER then
                    if roster then
                        ILM.MODULES.PointManager:UpdateRosterPoints(roster, awardValue, awardReason, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, false, self.note, self.awardType)

                        if ILM.GlobalConfigs:GetAnnounceAwardToGuild() then
                            UTILS.SendChatMessage(
                                string.format(
                                    ILM.L["Awarded %s points to %s player(s) for %s in <%s>"] .. note,
                                    awardValue, string.lower(ILM.L["All"]), CONSTANTS.POINT_CHANGE_REASONS.ALL[awardReason] or "",
                                    ILM.MODULES.RosterManager:GetRosterNameByUid(roster:UID())
                                ),
                                "GUILD"
                            )
                        end
                ...
                elseif self.context == CONSTANTS.ACTION_CONTEXT.SELECTED then
                    ...
                    if not profiles or #profiles == 0 then
                        LOG:Message(ILM.L["No players selected"])
                        LOG:Debug("UnifiedGUI_Standings(Award): profiles == 0")
                        return
                    end
                    if roster then
                        ILM.MODULES.PointManager:UpdatePoints(roster, profiles, awardValue, awardReason, CONSTANTS.POINT_MANAGER_ACTION.MODIFY, self.note, self.awardType)
            ...
            confirm = (function()
                local awardValue = tonumber(self.awardValue)
                if not awardValue then return ILM.L["Missing award value"] end
                local roster = ILM.MODULES.RosterManager:GetRosterByUid(self.roster)
                local points = UTILS.DecodePointTypeChangeName(roster:GetPointType(), self.awardType, true)
                if self.context == CONSTANTS.ACTION_CONTEXT.RAID then
                    return string.format(ILM.L["Award %s %s to everyone in raid."], awardValue, points)
                elseif self.context == CONSTANTS.ACTION_CONTEXT.ROSTER then
                    return string.format(ILM.L["Award %s %s to everyone in roster."], awardValue, points)
                elseif self.context == CONSTANTS.ACTION_CONTEXT.SELECTED then
                    local profiles = UnifiedGUI_Standings:GetSelection()
                    if not profiles then profiles = {} end
                    return string.format(ILM.L["Award %s %s to %s selected players."], awardValue, points, #profiles)
                end
            end),
            order = 14
        },
...
local function GenerateManagerOptions(self)
    return {
        decay_dkp_value = {
            name = ILM.L["Decay %"],
            desc = ILM.L["% that will be decayed."],
            type = "input",
        ...
        decay_dkp = {
            name = ILM.L["Decay"],
            desc = ILM.L["Execute decay for players based on context."],
            type = "execute",
            width = isElvUI and 0.5 or 0.575,
            func = (function(i)
                -- Decay Value
                ...
                if self.context == CONSTANTS.ACTION_CONTEXT.ROSTER then
                    ILM.MODULES.PointManager:UpdateRosterPoints(roster, decayValue, CONSTANTS.POINT_CHANGE_REASON.DECAY, CONSTANTS.POINT_MANAGER_ACTION.DECAY, not self.includeNegative, nil, self.decayType)
                elseif self.context == CONSTANTS.ACTION_CONTEXT.SELECTED then
                    local profiles = UnifiedGUI_Standings:GetSelection()
                    if not profiles or #profiles == 0 then
                        LOG:Message(ILM.L["No players selected"])
                    ...
                    ILM.MODULES.PointManager:UpdatePoints(roster, profiles, decayValue, CONSTANTS.POINT_CHANGE_REASON.DECAY, CONSTANTS.POINT_MANAGER_ACTION.DECAY, nil, self.decayType)
                else
            ...
            confirm = (function()
                local decayValue = tonumber(self.decayValue)
                if not decayValue then return ILM.L["Missing decay value"] end
                local roster = ILM.MODULES.RosterManager:GetRosterByUid(self.roster)
                local points = UTILS.DecodePointTypeChangeName(roster:GetPointType(), self.decayType, true)
                if self.context == CONSTANTS.ACTION_CONTEXT.RAID then
                    return ILM.L["Invalid context. You should not decay raid only."]
                elseif self.context == CONSTANTS.ACTION_CONTEXT.ROSTER then
                    return string.format(ILM.L["Decay %s%% %s to everyone in roster."], decayValue, points)
                elseif self.context == CONSTANTS.ACTION_CONTEXT.SELECTED then
                    local profiles = UnifiedGUI_Standings:GetSelection()
                    if not profiles then profiles = {} end
                    return string.format(ILM.L["Decay %s%% %s to %s selected players."], decayValue, points, #profiles)
                end
            end),
            order = 22
        }
...
local function verticalOptionsFeeder()
    ...
    if ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.MANAGER) then
...
local columnsDKP = {
    ...
    {   name = ILM.L["DKP"], width = 65, sort = LibStub("ScrollingTable").SORT_DSC, color = colorGreen },
    {   name = ILM.L["Spent"], width = 65 },
    {   name = ILM.L["Received"], width = 65 },
    ...
    events = {
        ...
        OnEnter = (function (rowFrame, cellFrame, data, cols, row, realrow, column, table, ...)
            ...
            if ST_GetIsLocked(rowData) then
                lockedString = "|c4eED3333" .. ILM.L["Locked"] .. "|r"
            end
            ...
            if #lootList > 0 then
                ...
                tooltip:AddLine(UTILS.ColorCodeText(ILM.L["No loot received"], "44ee44"))
            end
            -- Point History
            ...
            if #pointList > 0 then
                ...
                tooltip:AddLine(UTILS.ColorCodeText(ILM.L["No points received"], "44ee44"))
            end
            -- Display
            tooltip:Show()
            return status
        ...
        OnClick = function(rowFrame, cellFrame, data, cols, row, realrow, column, table, button, ...)
            ...
            ILM.GUI.Unified:RefreshOptionsPane()
            return true
        end
    }
}
...
local function initializeHandler()
    ...
    UnifiedGUI_Standings.RightClickMenu = ILM.UTILS.GenerateDropDownMenu(
        {
            {
                title = ILM.L["Add to standby"],
                func = (function()
                    if not ILM.MODULES.RaidManager:IsInRaid() then
                        LOG:Message(ILM.L["Not in raid"])
                        return
                    end
                    local profiles = UnifiedGUI_Standings:GetSelection()
                    local raid = ILM.MODULES.RaidManager:GetRaid()
                    ...
                    if roster ~= raid:Roster() then
                        LOG:Message(string.format(
                            ILM.L["You can only bench players from same roster as the raid (%s)."],
                            ILM.MODULES.RosterManager:GetRosterNameByUid(raid:Roster():UID())
                        ))
                        return
                    end

                    if ILM.MODULES.RaidManager:IsInProgressingRaid() then
                        if #profiles > 10 then
                            LOG:Message(string.format(
                                ILM.L["You can %s max %d players to standby at the same time to a %s raid."],
                                ILM.L["add"], 10, ILM.L["progressing"]
                            ))
                            return
                        end
                        ILM.MODULES.RaidManager:AddToStandby(ILM.MODULES.RaidManager:GetRaid(), profiles)
                    elseif ILM.MODULES.RaidManager:IsInCreatedRaid() then
                        if #profiles > 25 then
                            LOG:Message(string.format(
                                ILM.L["You can %s max %d players to standby at the same time to a %s raid."],
                                ILM.L["add"], 25, ILM.L["created"]
                            ))
                            return
                        end
                        for _, profile in ipairs(profiles) do
                            ILM.MODULES.StandbyStagingManager:AddToStandby(ILM.MODULES.RaidManager:GetRaid():UID(), profile:GUID())
                        end
                    end
                    refreshFn(true)
            ...
            {
                title = ILM.L["Remove from standby"],
                func = (function()
                    if not ILM.MODULES.RaidManager:IsInRaid() then
                        LOG:Message(ILM.L["Not in raid"])
                        return
                    ...
                    if roster ~= raid:Roster() then
                        LOG:Message(string.format(
                            ILM.L["You can only remove from bench players from same roster as the raid (%s)."],
                            ILM.MODULES.RosterManager:GetRosterNameByUid(raid:Roster():UID())
                        ))
                        return
                    end
                    ...
                    if ILM.MODULES.RaidManager:IsInProgressingRaid() then
                        if #profiles > 10 then
                            LOG:Message(string.format(
                                ILM.L["You can %s max %d players from standby at the same time to a %s raid."],
                                ILM.L["remove"], 10, ILM.L["progressing"]
                            ))
                            return
                        end
                        ILM.MODULES.RaidManager:RemoveFromStandby(ILM.MODULES.RaidManager:GetRaid(), profiles)
                    elseif ILM.MODULES.RaidManager:IsInCreatedRaid() then
                        if #profiles > 25 then
                            LOG:Message(string.format(
                                ILM.L["You can %s max %d players from standby at the same time to a %s raid."],
                                ILM.L["remove"], 25, ILM.L["created"]
                            ))
                            return
                        end
                        for _, profile in ipairs(profiles) do
                            ILM.MODULES.StandbyStagingManager:RemoveFromStandby(ILM.MODULES.RaidManager:GetRaid():UID(), profile:GUID())
                        end
                    end
                    refreshFn(true)
                    ILM.GUI.Unified:ClearSelection()
            ...
            {
                title = ILM.L["Remove from roster"],
                func = (function(i)
                    local profiles = UnifiedGUI_Standings:GetSelection()
                    local roster = ILM.MODULES.RosterManager:GetRosterByUid(UnifiedGUI_Standings.roster)
                    if roster == nil then
                    ...
                    if #profiles > 10 then
                        LOG:Message(string.format(
                            ILM.L["You can remove max %d players from roster at the same time."],
                            10
                        ))
                        return
                    end
                    ILM.MODULES.RosterManager:RemoveProfilesFromRoster(roster, profiles)
                    refreshFn(true)
                    ILM.GUI.Unified:ClearSelection()
                end),
                trustedOnly = true,
...
local function beforeShowHandler()
    ...
    if ILM.MODULES.RaidManager:IsInRaid() then
        HandleRosterChange(ILM.MODULES.RaidManager:GetRaid():Roster():UID())
...
local function storeHandler()
    ...
    local storage = ILM.GUI.Unified:GetStorage(UnifiedGUI_Standings.name)
...
local function dataReadyHandler()
    ...
    if not ILM.MODULES.RosterManager:GetRosterByUid(UnifiedGUI_Standings.roster) then
...
CONSTANTS.ACTION_CONTEXT_GUI = {
    [CONSTANTS.ACTION_CONTEXT.SELECTED] = ILM.L["Selected"],
    [CONSTANTS.ACTION_CONTEXT.ROSTER] = ILM.L["Roster"],
    [CONSTANTS.ACTION_CONTEXT.RAID] = ILM.L["Raid"],
}
