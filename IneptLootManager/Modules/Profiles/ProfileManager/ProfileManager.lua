...
function ProfileManager:Initialize()
    ...
    self._deProfile = ILM.MODELS.Profile:New(nil, ILM.L["Disenchanted"])
    ...
    self.db = ILM.MODULES.Database:Personal('profileManager', {
        pruneLog = {}
    })

    -- Register mutators
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.PROFILE.Update,
        (function(entry)
            ...
            if profileInternal then
                ...
                -- New mapping
                local profile = ILM.MODELS.Profile:New(entry, name, class, main)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.PROFILE.Remove,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.PROFILE.Link,
        (function(entry)
            ...
            local altProfile = self:GetProfileByGUID(altGUID)
            if not UTILS.typeof(altProfile, ILM.MODELS.Profile) then return end
            local mainProfile = self:GetProfileByGUID(mainGUID)
            if not UTILS.typeof(mainProfile, ILM.MODELS.Profile) then -- Unlink
                -- Check if our main exists
                local currentMainProfile = self:GetProfileByGUID(altProfile:Main())
                if not UTILS.typeof(currentMainProfile, ILM.MODELS.Profile) then return end
                ...
                if UTILS.typeof(self:GetProfileByGUID(mainProfile:Main()), ILM.MODELS.Profile) then return end
                -- Do not allow alt chaining if alt has alts
                if altProfile:HasAlts() then return end
                -- Unlink old main
                local currentMainProfile = self:GetProfileByGUID(altProfile:Main())
                if UTILS.typeof(currentMainProfile, ILM.MODELS.Profile) then
                    -- Remove main from this alt
                    altProfile:ClearMain()
                    -- Remove alt count from main
                    currentMainProfile:RemoveAlt(altGUID)
                ...
                for _,roster in pairs(rosters) do
                    if roster:IsProfileInRoster(altGUID) then
                        ...
                        ILM.MODULES.PointManager:AddFakePointHistory(roster, targets, pointSum, CONSTANTS.POINT_CHANGE_REASON.LINKING_OVERRIDE, entry:time(), entry:creatorFull())
                        ILM.MODULES.PointManager:AddFakePointHistory(roster, targets, spentSum, CONSTANTS.POINT_CHANGE_REASON.LINKING_OVERRIDE, entry:time(), entry:creatorFull(), nil, CONSTANTS.POINT_CHANGE_TYPE.SPENT)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.PROFILE.Lock,
        (function(entry)
...
function ProfileManager:NewProfile(GUID, name, class)
    ...
    if guidProfile then -- profile with this GUID already exists. Is this a rename?
        if nameProfile then -- profile with name and guid exists
            if guidProfile:GUID() == nameProfile:GUID() then -- same profile, same name - do nothing
                ...
                discard = true
                LOG:Debug("NewProfile(): guidProfile:GUID() ~= nameProfile:GUID()")
                warning = string.format(ILM.L["Two different profiles exist for target GUID %s (%s:%s) and name %s (%s:%s). Verify and clean up profiles before updating."], GUID, name, guidProfile:GUID(), guidProfile:Name(), nameProfile:GUID(), nameProfile:Name())
            end
        ...
        if nameProfile then -- name is used already by different profile? Warning
            ...
            warning = string.format(ILM.L["Profile %s already exists and is used by different GUID %s (%s). "], name, nameProfile:GUID(), nameProfile:Name())
        else -- New profile!
            discard = false
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.PROFILE.Update:new(GUID, name, class), true)
end
...
function ProfileManager:RemoveProfile(GUID)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.PROFILE.Remove:new(GUID), true)
end
...
local function PruneProfile(self, GUID, log)
    ...
    ILM.MODULES.LedgerManager:Remove(entry)
end
...
function ProfileManager:MarkAsAltByNames(alt, main)
    ...
    if not UTILS.typeof(altProfile, ILM.MODELS.Profile) then
        LOG:Error("Alt does not exist. Aborting")
        return
    end
    local mainProfile = self:GetProfileByName(main)
    -- Unlink
    if not UTILS.typeof(mainProfile, ILM.MODELS.Profile) then
        if altProfile:Main() ~= "" then
            ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.PROFILE.Link:new(altProfile:GUID(), nil), true)
            LOG:Message(string.format("%s: %s", ILM.L["Unlink Alt"], UTILS.ColorCodeText(altProfile:Name(), UTILS.GetClassColor(altProfile:Class()).hex)))
        else
            LOG:Error("Main does not exist.")
        end
    else -- Link
        -- Do not allow alt chaining if main is alt
        if UTILS.typeof(self:GetProfileByGUID(mainProfile:Main()), ILM.MODELS.Profile) then
        ...
        ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.PROFILE.Link:new(altProfile:GUID(), mainProfile:GUID()), true)
...
function ProfileManager:SetProfilesLock(profiles, lock)
    ...
    local entry = ILM.MODELS.LEDGER.PROFILE.Lock:new(profiles, lock and true or false)
...
function ProfileManager:PruneBelowLevel(minLevel, nop)
    ...
    local log = ILM.MODELS.ProfilePruneLog:New("level", nop)
    local prune
    if nop then
...
function ProfileManager:PruneRank(rank, nop)
    ...
    local log = ILM.MODELS.ProfilePruneLog:New("rank", nop)
    local prune
    if nop then
        LOG:Info("Pruning: No operation")
        prune = (function(GUID)
...
function ProfileManager:PruneUnguilded(nop)
    ...
    local log = ILM.MODELS.ProfilePruneLog:New("unguilded", nop)
    local prune
    if nop then
        LOG:Info("Pruning: No operation")
        prune = (function(GUID)
...
-- Publis API
ILM.MODULES.ProfileManager = ProfileManager
