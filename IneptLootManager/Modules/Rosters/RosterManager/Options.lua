...
local function SetRosterOption(name, option, value)
    ILM.MODULES.RosterManager:SetRosterConfiguration(name, option, value)
end

function RosterManagerOptions:Initialize()
    self.pointType = CONSTANTS.POINT_TYPE.DKP
    self.rosterName = ILM.MODULES.RosterManager:GenerateName()
    self.readOnly = not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.MANAGER)
    self.handlers = {
        general_name_get = (function(name)
            return name
        end),
        general_name_set = (function(old, new)
            ILM.MODULES.RosterManager:RenameRoster(old, new)
        end),
        general_remove_execute = (function(name)
            ILM.MODULES.RosterManager:DeleteRosterByName(name)
        end),
        general_fill_profiles_execute = (function(name)
            ...
            local roster = ILM.MODULES.RosterManager:GetRosterByName(name)
            ILM.MODULES.RosterManager:AddProfilesToRoster(roster, profileList)
        end),
        general_copy_execute = (function(name)
            if self.copy_source_name == nil then return end
            ILM.MODULES.RosterManager:Copy(self.copy_source_name, name, true, true, true, false)
        end),
    ...
    ILM.MODULES.LedgerManager:RegisterOnUpdate(function(lag, uncommitted)
        ...
        self:UpdateOptions()
        ILM.MODULES.ConfigManager:UpdateOptions(CONSTANTS.CONFIGS.GROUP.ROSTER)
    end)
end
...
function RosterManagerOptions:_Handle(cbtype, info, ...)
    ...
    if cbtype == CBTYPE.CONFIRMATION then
        if ILM.MODULES.RaidManager:IsInActiveRaid() then
            local raid = ILM.MODULES.RaidManager:GetRaid()
            if raid:Roster() == ILM.MODULES.RosterManager:GetRosterByName(roster_name) and CONFIRMATION_GROUPS[group] then
                return ILM.L["You are changing roster settings during active raid. You can continue without any issues however the settings will not get applied until you start a new one."]
            end
        end
    ...
    for _, slot in ipairs(CONSTANTS.INVENTORY_TYPES_SORTED) do
        ...
        for _, ivalues in ipairs(valuesWithDesc) do
            args[prefix .. "_" .. ivalues.type] = {
                ...
                set = (function(i, v)
                    if self.readOnly then return end
                    ILM.MODULES.RosterManager:SetRosterDefaultSlotTierValue(roster, slot.type, ivalues.type, tonumber(v))
                end),
        ...
        if icon then
            ...
            for _, ivalues in ipairs(valuesWithDesc) do
                args[prefix .. "_" .. ivalues.type] = {
                    ...
                    set = (function(i, v)
                        if self.readOnly then return end
                        ILM.MODULES.RosterManager:SetRosterItemTierValue(roster, id, ivalues.type, tonumber(v))
                    end),
                    get = (function(i)
            ...
            args[prefix .. "remove"] = {
                name = "x",
                desc = ILM.L["Remove override"],
                -- image = icon,
                order = order,
                width = 0.30,
                type = "execute",
                func = (function() ILM.MODULES.RosterManager:RemoveRosterItemOverride(roster, id) end)
            }
            order = order + 6
        end
    end
...
local function generateBossKillAwardValueInputField(self, roster, info, instanceName, name, difficultyId, order, isHardMode, fields)
    local nameModified = ILM.DifficultyIDsMap[difficultyId] .. (isHardMode and (" " .. ILM.UTILS.ColorCodeText(ILM.L["Hard Mode"], "ee4444")) or "")
    local width
    ...
    return {
        ...
        set = (function(i, v)
            if self.readOnly then return end
            ILM.MODULES.RosterManager:SetRosterBossKillBonusValue(name, info.id, difficultyId, isHardMode, v)
        end),
        get = (function(i)
...
local function boss_kill_award_values(self, roster, name)
    ...
    -- Common
    local order
    for expansion, encounters in pairs(ILM.EncounterIDs) do
        ...
        for _, instanceData in ipairs(encounters) do
            ...
            for _, info in ipairs(instanceData.data) do
                ...
                for _,difficultyId in ipairs(instanceData.difficulty) do
                    local hardmode = ILM.EncounterHasHardMode[info.id] and true or false
                    order = order + 1
...
function RosterManagerOptions:GenerateRosterOptions(name)
    ...
    local options = {
        ...
        args = {
            general = {
                name = ILM.L["General settings"],
                type = "group",
                order = 1,
                args = {
                    name = {
                        name = ILM.L["Name"],
                        desc = ILM.L["Change roster name."],
                        type = "input",
                        width = 1,
                        order = 0
                    },
                    ...
                    round_decimals = {
                        name = ILM.L["Rounding"],
                        desc = ILM.L["Round to selected number of decimals"],
                        type = "select",
                        -- style = "radio",
                        width = 1,
                    ...
                    copy = {
                        name = ILM.L["Copy settings"],
                        desc = ILM.L["Copy settings from selected roster."],
                        type = "execute",
                        confirm = true,
                        disabled = disableManage,
                        order = 98
                    ...
                    copy_source = {
                        name = ILM.L["Copy source"],
                        desc = ILM.L["Copy settings from selected roster."],
                        type = "select",
                        values = (function()
                            local v = {}
                            local r = ILM.MODULES.RosterManager:GetRosters()
                    ...
                    remove = {
                        name = ILM.L["Remove"],
                        desc = ILM.L["Removes current roster."],
                        type = "execute",
                        confirm = true,
                        disabled = disableManage,
                        order = 101
                    ...
                    boss_kill_bonus_value = {
                        name = ILM.L["Default Boss Kill Bonus Value"],
                        type = "input",
                        order = 8,
                    ...
                    on_time_bonus_value = {
                        name = ILM.L["On Time Bonus Value"],
                        type = "input",
                        order = 9,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                        disabled = disableManage,
                    ...
                    raid_completion_bonus = {
                        name = ILM.L["Raid Completion Bonus"],
                        type = "toggle",
                        order = 7,
                        disabled = disableManage,
                        width = 1
                    ...
                    raid_completion_bonus_value = {
                        name = ILM.L["Raid Completion Value"],
                        type = "input",
                        order = 10,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                        disabled = disableManage,
                    ...
                    interval_bonus_time = {
                        name = ILM.L["Interval Time"],
                        desc = ILM.L["Interval in [minutes] to award bonus points"],
                        type = "input",
                        order = 12,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                        disabled = disableManage,
                    ...
                    auto_award_include_bench =  {
                        name = ILM.L["Include bench"],
                        desc = ILM.L["Include benched players in all auto-awards"],
                        type = "toggle",
                        order = 14,
                        disabled = disableManage,
                        -- width = 2
                    ...
                    auto_award_online_only =  {
                        name = ILM.L["Online only"],
                        desc = ILM.L["Award points only to online players"],
                        type = "toggle",
                        order = 15,
                        -- disabled = disableManage,
                        disabled = true,
                    ...
                    auto_award_same_zone_only =  {
                        name = ILM.L["Same zone only"],
                        desc = ILM.L["Award points only to players in same zone"],
                        type = "toggle",
                        order = 16,
                        -- disabled = disableManage,
                        disabled = true,
                    ...
                    point_caps_header = {
                        name = ILM.L["Point caps"],
                        type = "header",
                        order = 17,
                        width = "full"
                    },
                    weekly_reset_timezone = {
                        name = ILM.L["Weekly reset timezone"],
                        desc = ILM.L["Select weekly reset timezone. EU: Wed 07:00 GMT or US: Tue 15:00 GMT"],
                        type = "select",
                        -- style = "radio",
                        disabled = disableManage,
                        order = 18,
                    ...
                    weekly_cap = {
                        name = ILM.L["Weekly cap"],
                        desc = ILM.L["Maximum point cap player can receive per raid week. Set to 0 to disable."],
                        type = "input",
                        disabled = disableManage,
                        order = 19,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                    ...
                    hard_cap = {
                        name = ILM.L["Hard cap"],
                        desc = ILM.L["Maximum point cap that player can have. Set to 0 to disable."],
                        type = "input",
                        disabled = disableManage,
                        order = 20,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                    ...
                    allow_self_bench_subscribe =  {
                        name = ILM.L["Allow subscription"],
                        desc = ILM.L["Allow players to subscribe to the bench through Raids menu"],
                        type = "toggle",
                        order = 25,
                        disabled = disableManage,
                        width = 1
                    ...
                    auto_bench_leavers =  {
                        name = ILM.L["Auto bench leavers"],
                        desc = ILM.L["Put players leaving raid on bench instead of removing them. To remove them completely they will need to be removed manually from the bench."],
                        type = "toggle",
                        order = 26,
                        disabled = disableManage,
                        width = 1
                    ...
                    bench_multiplier =  {
                        name = ILM.L["Bench multiplier"],
                        desc = ILM.L["Point award multiplier for players on bench."],
                        type = "input",
                        order = 27,
                        disabled = disableManage,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
            ...
            auction = {
                ...
                args = {
                    ...
                    minimum_points = {
                        name = ILM.L["Minimum standing"],
                        desc = ILM.L["Minimum standing required to be allowed to bid."],
                        type = "input",
                        disabled = disableManage,
                        pattern = CONSTANTS.REGEXP_FLOAT,
                        width = 1,
                    ...
                    cancelpass = {
                        name = ILM.L["Always allow cancel/pass"],
                        desc = ILM.L["Enable to always allow cancelling or passing bids. Affects Open-Ascending Auction."],
                        type = "toggle",
                        disabled = disableManage,
                        width = 2,
                        order = 12.1
                    ...
                    auction_time = {
                        name = ILM.L["Auction length"],
                        desc = ILM.L["Auction length in seconds."],
                        type = "input",
                        disabled = disableManage,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                        width = 1,
                    ...
                    antisnipe_time = {
                        name = ILM.L["Anti-snipe time"],
                        desc = ILM.L["Time in seconds by which auction will be extended if bid is received during last 10 seconds."],
                        type = "input",
                        disabled = disableManage,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                        width = 1,
                    ...
                    linear_time_multiply = {
                        name = ILM.L["Multiply time"],
                        desc = ILM.L["Multiply auction time by the number of items to increase auction time lineary."],
                        type = "toggle",
                        disabled = disableManage,
                        width = 1,
                        order = 16
                    ...
                    roll_time = {
                        name = ILM.L["Roll"] .. " " .. ILM.L["Time"],
                        desc = ILM.L["Roll"] .. " " .. ILM.L["Time"],
                        type = "input",
                        disabled = disableManage,
                        pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
                        width = 1,
            ...
            default_slot_values = {
                name = ILM.L["Default slot values"],
                type = "group",
                order = 3,
    ...
    options.args.auction.args.auction_type = {
        name = ILM.L["Auction type"],
        desc = ILM.L["|cff00ee44Open:|r English Auction with highest bidder announcement. Highest bidder wins. Two players can not bid same value. Additionally always allows bidding base to accomodate for Swedish Auction flavor.\n\n|cff00ee44Anonymous Open:|r Same as Open but highest bidder name is not disclosed.\n\n|cff00ee44Sealed:|r Bids are not announced. Highest bidder wins.\n\n|cff00ee44Vickrey:|r Same as sealed but winner pays with second-highest bid."],
        type = "select",
        disabled = disableManage,
        order = 4,
    ...
    options.args.general.args.named_buttons_toggle = {
        name = ILM.L["Use named buttons"],
        desc = ILM.L["Will display names of the buttons instead of values in bidding UI"],
        type = "toggle",
        order = 29,
        disabled = disableManage,
        width = 1
    ...
    for _, tier in ipairs(CONSTANTS.SLOT_VALUE_TIERS_ORDERED) do
        options.args.general.args["named_button_tier_" .. tostring(tier)] = {
            ...
            type = "input",
            set = (function(i, v) ILM.MODULES.RosterManager:SetFieldName(roster, tier, v) end),
    ...
    if roster:GetPointType() == CONSTANTS.POINT_TYPE.DKP then
        options.args.auction.args.base_always = {
            name = ILM.L["Always allow Base bids"],
            desc = ILM.L["Enable to allow base bids even if higher bids are present if player didn't bid yet. Affects Open-Ascending Auction."],
            type = "toggle",
            disabled = disableManage,
            width = 1,
            order = 12.1
        ...
        options.args.auction.args.allin_always = {
            name = ILM.L["Always allow All-In bids"],
            desc = ILM.L["Enable to always allow All-In bids. Affects Open-Ascending Auction."],
            type = "toggle",
            disabled = disableManage,
            width = 1,
            order = 12.2
        ...
        options.args.auction.args.equalbids = {
            name = ILM.L["Allow equal bids"],
            desc = ILM.L["Enable to allow equal bids. Affects Open-Ascending Auction."],
            type = "toggle",
            disabled = disableManage,
            width = 1,
            order = 12.3
        ...
        options.args.auction.args.always0 = {
            name = ILM.L["Always allow 0 bids"],
            desc = ILM.L["Enable to always allow 0 bids. Affects ascending item value mode."],
            type = "toggle",
            disabled = disableManage,
            width = 1,
            order = 12.4
...
options.args.auction.args.allow_below_min_standings = {
        ...
        options.args.auction.args.minimal_increment = {
            name = ILM.L["Minimal increment"],
            desc = ILM.L["Minimal value increment for open auction mode."],
            type = "input",
            disabled = disableManage,
            pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
            width = 1,
        ...
        options.args.auction.args.tax = {
            name = ILM.L["Tax"],
            desc = ILM.L["Additional cost (tax) to add to the award value."],
            type = "input",
            disabled = disableManage,
            pattern = CONSTANTS.REGEXP_FLOAT_POSITIVE,
            width = 1,
...
function RosterManagerOptions:UpdateOptions()
    local options = {
        new = { -- Global options -> Create New Roster
            name = ILM.L["Create"],
            desc = ILM.L["Creates new roster with default configuration"],
            type = "execute",
            func = function() ILM.MODULES.RosterManager:NewRoster(self.pointType, self.rosterName); self.rosterName = ILM.MODULES.RosterManager:GenerateName() end,
            disabled = (function() return not ILM.MODULES.ACL:CheckLevel(CONSTANTS.ACL.LEVEL.MANAGER) end),
            order = 1
        },
        roster_name = {
            name = ILM.L["Roster name"],
            desc = ILM.L["Roster Name"],
            type = "input",
    ...
    ILM.MODULES.ConfigManager:Register(CONSTANTS.CONFIGS.GROUP.ROSTER, options, true)
end

ILM.OPTIONS.RosterManager = RosterManagerOptions
