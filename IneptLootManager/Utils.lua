...
function UTILS.ColorCodeAndLocalizeClass(className)
    return ColorCodeText(ILM.L[className], GetClassColor(className).hex);
end
...
function UTILS.GetGUIDFromEntry(e)
    if typeof(e, ILM.MODELS.Profile) then
...
function UTILS.putListInTooltip(data, tooltip, inLine, max, dataProvider, autoWrap)
    ...
    if notIncludedEntries > 0 then
        tooltip:AddLine(notIncludedEntries .. ILM.L[" more"], nil, nil, nil, autoWrap and true or false)
    end
end
...
local greenYes = ColorCodeText(ILM.L["Yes"], "00cc00")
function UTILS.GreenYes()
    return greenYes
end

local redNo = ColorCodeText(ILM.L["No"], "cc0000")
...
function UTILS.GenerateDropDownMenu(structure, isAssistant, isManager, frame)
    frame = frame or CreateFrame("Frame", "ILM_Generic_Menu_DropDown" .. tostring(menuCounter), UIParent, "UIDropDownMenuTemplate")
    ...
    if icon then
        ...
        frame:SetScript("OnEnter", function()
            ...
            if note then
                ...
                GameTooltip:AddTexture(ILM_ICON_DARK)
            end
            GameTooltip:Show()
        end)
...
function UTILS.SendChatMessage(message, channel, _, target)
    ...
    SendChatMessage("[ILM] " .. tostring(message), channel, nil, target)
end
...
CONSTANTS.ITEM_QUALITY = {
    [0] = ColorCodeText(ILM.L["Poor"], "9d9d9d"),
    [1] = ColorCodeText(ILM.L["Common"], "ffffff"),
    [2] = ColorCodeText(ILM.L["Uncommon"], "1eff00"),
    [3] = ColorCodeText(ILM.L["Rare"], "0070dd"),
    [4] = ColorCodeText(ILM.L["Epic"], "a335ee"),
    [5] = ColorCodeText(ILM.L["Legendary"], "ff8000"),
}
CONSTANTS.LOOT_ROLL_TYPE_ANY = -2
CONSTANTS.LOOT_ROLL_TYPE_IGNORE = -1
CONSTANTS.ROLL_TYPE = {
    [CONSTANTS.LOOT_ROLL_TYPE_ANY]      = ColorCodeText(ILM.L["Any"], "ff8000"),
    [CONSTANTS.LOOT_ROLL_TYPE_IGNORE]   = ColorCodeText(ILM.L["Do Nothing"], "9d9d9d"),
