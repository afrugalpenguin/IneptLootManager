...
function AuctionHistoryManager:Initialize()
    ...
    self.db = ILM.MODULES.Database:Personal('auctionHistory2', {
        stack = {},
        -- config
        enable = true,
        post_bids_channel = 5
    ...
    local options = {
        auction_history_header = {
            type = "header",
            name = ILM.L["Auctioning - History"],
            order = 50
        },
        auction_history_store_bids = {
            name = ILM.L["Store bids"],
            desc = ILM.L["Store finished auction bids information."],
            type = "toggle",
        ...
        auction_history_post_bids_channel = {
            name = ILM.L["Post channel"],
            desc = ILM.L["Channel for posting bids."],
            type = "select",
            values = CHANNELS,
...
function AuctionHistoryManager:PostData(data)
    if data then
        ...
        if noBids then
            UTILS.SendChatMessage(ILM.L["No bids"], channel)
        end
    end
end
...
ILM.MODULES.AuctionHistoryManager = AuctionHistoryManager