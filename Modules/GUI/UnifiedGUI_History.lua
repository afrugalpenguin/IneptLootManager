...
CONSTANTS.HISTORY_TYPES_GUI = {
    [CONSTANTS.HISTORY_TYPE.ALL] = ILM.L["All"],
    [CONSTANTS.HISTORY_TYPE.LOOT] = ILM.L["Loot"],
    [CONSTANTS.HISTORY_TYPE.POINT] = ILM.L["Point"]
}
...
local tooltipPool = CreateObjectPool(function(pool)
    ...
    return CreateFrame("GameTooltip", "ILMHistoryTT" .. tostring(pool.nextId), UIParent, "GameTooltipTemplate")
...
local UnifiedGUI_History = {
    ...
    nil, 3),
    -- tooltip = CreateFrame("GameTooltip", "ILMUnifiedGUIHistoryDialogTooltip", UIParent, "GameTooltipTemplate"),
    tooltip = GameTooltip,
...
local function GenerateUntrustedOptions(self)
    ...
    local profileNameMap = { [ILM.L["-- All --"]] = ILM.L["-- All --"]}
    local profileList = {ILM.L["-- All --"]}
...
local function buildLootTooltip(tooltip, loot, itemLink, detailsMode)
    ...
    if loot then
        local profile = ILM.MODULES.ProfileManager:GetProfileByGUID(UTILS.getGuidFromInteger(loot:Creator()))
        local name
        if profile then
        ...
        tooltip:AddDoubleLine(ILM.L["Awarded by"], name)
        local auction = ILM.MODULES.AuctionHistoryManager:GetByUUID(loot:Entry():uuid())
        if detailsMode and auction then
            ...
            for bidder, bid in pairs(auction.bids) do
                ...
                local bidderProfile = ILM.MODULES.ProfileManager:GetProfileByName(bidder)
...
local function buildPointTooltip(tooltip, history)
    ...
    tooltip:AddDoubleLine(ILM.L["Affected players:"], tostring(numProfiles))
    if not profiles or numProfiles == 0 then
        tooltip:AddLine(ILM.L["None"])
    else
    ...
    if note ~= "" then
        ...
        if numNote then
            note = ILM.EncounterIDsMap[numNote] or note
        end
        tooltip:AddDoubleLine(ILM.L["Note"] .. "", note)
    end
    local profile = ILM.MODULES.ProfileManager:GetProfileByGUID(UTILS.getGuidFromInteger(history:Creator()))
    local name
    if profile then
        name = UTILS.ColorCodeText(profile:ShortName(), UTILS.GetClassColor(profile:Class()).hex)
    else
...
local tableStructure = {
    ...
    columns = {
        ...
        {name = ILM.L["Info"],  width = 217 },
        {name = ILM.L["Value"], width = 85, color = colorGreen,
        ...
        {name = ILM.L["Date"],  width = 205, sort = LibStub("ScrollingTable").SORT_DSC},
        {name = ILM.L["Player"],width = 95,
...
local function tableDataFeeder()
    ...
    if UnifiedGUI_History.historyType ~= CONSTANTS.HISTORY_TYPE.POINT then
        ...
        if UnifiedGUI_History.pendingLoot then
            return {{cols = { {value = ""}, {value = ""}, {value = ILM.L["Loading..."]}, {value = ""}, {value = nil}, {value = nil} }}}
        end
        ...
        for _,lootData in ipairs(displayedLoot) do
            ...
            local row = {cols = {
                ...
                {value = value},
                {value = date(ILM.L["%Y/%m/%d %H:%M:%S (%A)"], loot:Timestamp())},
...
local function initializeHandler()
    ...
    UnifiedGUI_History.RightClickMenu = ILM.UTILS.GenerateDropDownMenu(
        {
            {
                ...
                func = (function()
                    ...
                    for _, loot in pairs(selectedLoot) do
                        ILM.MODULES.LedgerManager:Remove(loot:Entry())
                        ILM.MODULES.EventManager:DispatchEvent(CONSTANTS.EVENTS.GLOBAL_LOOT_REMOVED, {
                    ...
                    for _, history in pairs(selectedHistory) do
                        ILM.MODULES.LedgerManager:Remove(history:Entry())
                    end
...
local function storeHandler()
    ...
    local storage = ILM.GUI.Unified:GetStorage(UnifiedGUI_History.name)
...
local function dataReadyHandler()
    ...
    if not ILM.MODULES.RosterManager:GetRosterByUid(UnifiedGUI_History.roster) then
