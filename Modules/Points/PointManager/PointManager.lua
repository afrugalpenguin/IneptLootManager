...
local function update_profile_standings(mutate, roster, targets, value, reason, changeType, timestamp, pointHistoryEntry, isGUID, alreadyApplied)
    ...
    for _,target in ipairs(targets) do
        ...
        if not roster:IsProfileInRoster(GUID) then
            ...
            local targetProfile = ILM.MODULES.ProfileManager:GetProfileByGUID(GUID)
            if targetProfile and not targetProfile:IsLocked() then
                ...
                if targetProfile:Main() == "" then -- is main
                    ...
                    mainProfile = ILM.MODULES.ProfileManager:GetProfileByGUID(targetProfile:Main())
                end
                -- Check if we should schedule it for alert
                ILM.MODULES.EventManager:DispatchEvent(
...
local function apply_mutator(entry, mutate)
    ...
    local pointHistoryEntry = ILM.MODELS.PointHistory:New(entry)
...
local function apply_roster_mutator(entry, mutate)
    ...
    local pointHistoryEntry = ILM.MODELS.PointHistory:New(entry, profiles)
    roster:AddRosterPointHistory(pointHistoryEntry)
...
local function apply_raid_mutator(self, entry, mutate)
    ...
    local pointHistoryEntry = ILM.MODELS.PointHistory:New(entry, players)
    ...
    if entry:standby() and (#playersOnStandby > 0) then
        ...
        pointHistoryEntry = ILM.MODELS.PointHistory:New(entry, playersOnStandby, nil, newValue, CONSTANTS.POINT_CHANGE_REASON.STANDBY_BONUS)
...
function PointManager:Initialize()
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.POINTS.Modify,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.POINTS.Decay,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.POINTS.ModifyRoster,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.POINTS.DecayRoster,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.POINTS.ModifyRaid,
        (function(entry)
...
function PointManager:UpdatePoints(roster, targets, value, reason, action, note, pointChangeType, forceInstant)
    ...
    if not typeof(roster, ILM.MODELS.Roster) then
    ...
    if typeof(targets, ILM.MODELS.Profile) or type(targets) == "number" or type(targets) == "string" then
        targets = { targets }
    ...
    if action == CONSTANTS.POINT_MANAGER_ACTION.MODIFY then
        entry = ILM.MODELS.LEDGER.POINTS.Modify:new(uid, targets, value, reason, note, isSpent)
    elseif action == CONSTANTS.POINT_MANAGER_ACTION.SET then
        entry = ILM.MODELS.LEDGER.POINTS.Set:new(uid, targets, value, reason, note, isSpent)
    elseif action == CONSTANTS.POINT_MANAGER_ACTION.DECAY then
        ...
        entry = ILM.MODELS.LEDGER.POINTS.Decay:new(uid, targets, value, reason, note, pointChangeType)
    end
...
function PointManager:UpdateRosterPoints(roster, value, reason, action, ignoreNegatives, note, pointChangeType, forceInstant)
    ...
    if action == CONSTANTS.POINT_MANAGER_ACTION.MODIFY then
        local isSpent = (pointChangeType == CONSTANTS.POINT_CHANGE_TYPE.SPENT)
        entry = ILM.MODELS.LEDGER.POINTS.ModifyRoster:new(uid, value, reason, note, isSpent)
    -- elseif action == CONSTANTS.POINT_MANAGER_ACTION.SET then
    ...
    elseif action == CONSTANTS.POINT_MANAGER_ACTION.DECAY then
        ...
        entry = ILM.MODELS.LEDGER.POINTS.DecayRoster:new(uid, value, reason, ignoreNegatives, note, pointChangeType)
    end
...
function PointManager:UpdateRaidPoints(raid, value, reason, action, note, pointChangeType, forceInstant)
    ...
    if not typeof(raid, ILM.MODELS.Raid) then
    ...
    if action == CONSTANTS.POINT_MANAGER_ACTION.MODIFY then
        local isSpent = (pointChangeType == CONSTANTS.POINT_CHANGE_TYPE.SPENT)
        entry = ILM.MODELS.LEDGER.POINTS.ModifyRaid:new(uid, value, reason, note, includeBench, isSpent)
    end
...
function PointManager:RemovePointChange(pointHistory, forceInstant)
    ...
    if not typeof(pointHistory, ILM.MODELS.PointHistory) then
        LOG:Error("PointManager:RemovePointChange(): Missing valid point history")
        return
    end

    ILM.MODULES.LedgerManager:Remove(pointHistory:Entry(), forceInstant)
end
...
function PointManager:UpdatePointsDirectly(roster, targets, value, reason, pointChangeType, timestamp, creator)
    ...
    local pointHistoryEntry = ILM.MODELS.FakePointHistory:New(targets, timestamp, value, reason, creator, pointChangeType)
    roster:AddRosterPointHistory(pointHistoryEntry)
...
function PointManager:AddPointHistory(roster, targets, pointHistoryEntry)
    ...
    for _,target in ipairs(targets) do
        if roster:IsProfileInRoster(target) then
            local targetProfile = ILM.MODULES.ProfileManager:GetProfileByGUID(target)
...
function PointManager:AddFakePointHistory(roster, targets, value, reason, timestamp, creator, note, pointChangeType)
    ...
    -- local isSpent = (pointChangeType == CONSTANTS.POINT_CHANGE_TYPE.SPENT)
    local pointHistoryEntry = ILM.MODELS.FakePointHistory:New(targets, timestamp, value, reason, creator, note, pointChangeType)
    roster:AddRosterPointHistory(pointHistoryEntry)
    for _,target in ipairs(targets) do
        if roster:IsProfileInRoster(target) then
            local targetProfile = ILM.MODULES.ProfileManager:GetProfileByGUID(target)
...
CONSTANTS.POINT_CHANGE_REASONS = {
    GENERAL = {  -- Exposed through GUI dropdown, can be localized
        [CONSTANTS.POINT_CHANGE_REASON.ON_TIME_BONUS] = ILM.L["On Time Bonus"],
        [CONSTANTS.POINT_CHANGE_REASON.BOSS_KILL_BONUS] = ILM.L["Boss Kill Bonus"],
        [CONSTANTS.POINT_CHANGE_REASON.RAID_COMPLETION_BONUS] = ILM.L["Raid Completion Bonus"],
        [CONSTANTS.POINT_CHANGE_REASON.PROGRESSION_BONUS] = ILM.L["Progression Bonus"],
        [CONSTANTS.POINT_CHANGE_REASON.STANDBY_BONUS] = ILM.L["Standby Bonus"],
        [CONSTANTS.POINT_CHANGE_REASON.UNEXCUSED_ABSENCE] = ILM.L["Unexcused absence"],
        [CONSTANTS.POINT_CHANGE_REASON.CORRECTING_ERROR] = ILM.L["Correcting error"],
        [CONSTANTS.POINT_CHANGE_REASON.MANUAL_ADJUSTMENT] = ILM.L["Manual adjustment"],
        [CONSTANTS.POINT_CHANGE_REASON.ZERO_SUM_AWARD] = ILM.L["Zero-Sum award"],
    },
    INTERNAL = { -- Not exposed directly to GUI
        [CONSTANTS.POINT_CHANGE_REASON.IMPORT] = ILM.L["Import"],
        [CONSTANTS.POINT_CHANGE_REASON.DECAY] = ILM.L["Decay"],
        [CONSTANTS.POINT_CHANGE_REASON.INTERVAL_BONUS] = ILM.L["Interval Bonus"],
        [CONSTANTS.POINT_CHANGE_REASON.LINKING_OVERRIDE] = ILM.L["Linking override"],
        [CONSTANTS.POINT_CHANGE_REASON.ROSTER_JOIN] = ILM.L["Joining roster"],
    }
}
...
function PointManager:DebugAddPoints()
    local rosters = ILM.MODULES.RosterManager:GetRosters()
