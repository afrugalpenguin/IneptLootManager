...
function RosterManager:Initialize()
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.Create,
        (function(entry)
            ...
            local roster = ILM.MODELS.Roster:New(uid, pointType, self.db.raidsForFullAttendance, self.db.attendanceWeeksWindow)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.Delete,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.Rename,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.CopyData,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.UpdateConfigSingle,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.UpdateDefaultSingle,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.UpdateOverrides,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.UpdateOverridesSingle,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.RemoveOverrides,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.UpdateProfiles,
        (function(entry)
                ...
                for _, iGUID in ipairs(profiles) do
                    ...
                    if profile then
                        ...
                        if profile:Main() ~= "" then
                            ...
                            ILM.MODULES.PointManager:AddFakePointHistory(roster, { GUID }, roster:Standings(profile:Main()), CONSTANTS.POINT_CHANGE_REASON.LINKING_OVERRIDE, entry:time(), entry:creatorFull())
                        else
                            if initialize then
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.BossKillBonus,
        (function(entry)
    ...
    ILM.MODULES.LedgerManager:RegisterEntryType(
        ILM.MODELS.LEDGER.ROSTER.FieldRename,
        (function(entry)
    ...
    self.db = ILM.MODULES.Database:Personal('rosterManager', {
    ...
    local options = {
        attendance_header = {
            type = "header",
            name = ILM.L["Attendance"],
            order = 20
        },
        attendance_max = {
            name = ILM.L["Raids needed in reset"],
            desc = ILM.L["Provide number of raids needed for 100% attendance in a weekly reset. Between 1 - 50 raids. Defaults to 2. Requires /reload."],
            type = "range",
            min = 1,
            max = 50,
            softMin = 1,
          ...
          attendance_window = {
            name = ILM.L["Average weeks"],
            desc = ILM.L["Provide number of weeks that will be accounted for attendance. Between 1 - 1000 weeks. Defaults to 10. Requires /reload."],
            type = "range",
            min = 1,
            max = 1000,
            softMin = 1,
          ...
          export_rosters = {
            name = ILM.L["Select Rosters to display"],
            type = "multiselect",
            set = function(i, k, v)
            ...
            values = function()
                local roster_list = {}
                for uid, name in pairs(ILM.MODULES.RosterManager:GetRostersUidMap()) do
...
function RosterManager:NewRoster(pointType, name)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.Create:new(uid, name, pointType), true)
end
...
function RosterManager:DeleteRosterByName(name)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.Delete:new(roster:UID()), true)
end
...
function RosterManager:RenameRoster(old, new)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.Rename:new(o:UID(), new), true)
end
...
function RosterManager:Copy(source, target, config, defaults, overrides, profiles)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.CopyData:new(s:UID(), t:UID(), config, defaults, overrides, profiles), true)
end
...
function RosterManager:SetRosterConfiguration(name, option, value)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.UpdateConfigSingle:new(roster:UID(), option, value), true)
end
...
function RosterManager:SetRosterDefaultSlotTierValue(nameOrRoster, slot, tier, value)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.UpdateDefaultSingle:new(roster:UID(), slot, tier, value), true)
end
...
function RosterManager:SetFieldName(nameOrRoster, field, name)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.FieldRename:new(roster:UID(), field, name), true)
end
...
function RosterManager:SetRosterItemValues(nameOrRoster, itemId, values)
    ...
    if sanitizedValues then
        ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.UpdateOverrides:new(roster:UID(), itemId, sanitizedValues), true)
    end
end
...
function RosterManager:RemoveRosterItemOverride(nameOrRoster, itemId)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.RemoveOverrides:new(roster:UID(), itemId), true)
end
...
function RosterManager:SetRosterItemTierValue(nameOrRoster, itemId, tier, value)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.UpdateOverridesSingle:new(roster:UID(), itemId, tier, value), true)
end
...
function RosterManager:SetRosterBossKillBonusValue(nameOrRoster, encounterId, difficultyId, isHardMode, value)
    ...
    ILM.MODULES.LedgerManager:Submit(ILM.MODELS.LEDGER.ROSTER.BossKillBonus:new(roster:UID(), encounterId, difficultyId, isHardMode, value), true)
end

function RosterManager:AddProfilesToRoster(roster, profiles)
    LOG:Trace("RosterManager:AddProfilesToRoster()")
    if not UTILS.typeof(roster, ILM.MODELS.Roster) then
    ...
    local entry = ILM.MODELS.LEDGER.ROSTER.UpdateProfiles:new(roster:UID(), profiles, false)
    ...
    ILM.MODULES.LedgerManager:Submit(entry, true)
end
...
function RosterManager:RemoveProfilesFromRoster(roster, profiles)
    ...
    local entry = ILM.MODELS.LEDGER.ROSTER.UpdateProfiles:new(roster:UID(), profiles, true)
...
function RosterManager:AddFromRaidToRoster(roster)
    ...
    if #missingProfiles > 0 then
        LOG:Message(ILM.L["Adding missing %s players to current roster"], #missingProfiles)
...
ILM.MODULES.RosterManager = RosterManager
